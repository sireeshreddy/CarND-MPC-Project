...
public:
    ...
    void operator()(ADvector& fg, const ADvector& vars) {
        ...
        // Cost for cross-track error, orientation error and velocity error.
        ...
        // Cost for actuator values
        for (unsigned int t = 0; t < N - 1; t++) {
            fg[0] += 5*CppAD::pow(vars[delta_start + t], 2);
            fg[0] += 5*CppAD::pow(vars[a_start + t], 2);
        }
        
        // Cost for actuator accelerations (difference between consecutive actuations)
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
    ...
    // The upper and lower steering limits are set to -30 and 30 degrees.
